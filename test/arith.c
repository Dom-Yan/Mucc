...
int main() {
  ...
  // ~, << and >> promote char and short operands to int.
  ASSERT(4, sizeof(~(char)1));
  ASSERT(4, sizeof((char)1 << 1));
  ASSERT(-69, ({ unsigned char c = 68; ~c; }));
  ASSERT(65467, ({ unsigned char c = 68; unsigned u = (unsigned short)~c; u; }));
  ASSERT(4080, ({ unsigned char c = 255; c << 4; }));
